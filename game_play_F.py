...
coin_image = [pygame.transform.scale(coin,(70,70)) for coin in coins_iamge1]
...
coin_rects = []
pause = False
pos_rand_coins_y = random.randint(150,300)
neg_rand_coins_y =  random.randint(500,620)
data = {'coin_num': 0}
def save_data():
...
def draw_coins():
    global coins_x, moves_coins, coin_num, coins_y, respawn_coins, coin_rects, coins_collected, coin_image

    # تحريك العملات على المحور X
    coins_x -= 15

    # إنشاء مستطيلات للتصادم مع العملات
    coin_rects = [
    ...
    # التحقق من تصادم اللاعب مع العملات وتحديث حالة الجمع
    ...
    # إعادة تعيين موضع العملات عندما تخرج من الشاشة
    if coins_x < -300:
        coins_x = screenw + 200  # إعادة تعيين إحداثيات X إلى الجانب الأيمن من الشاشة
        coins_y = y + 200  # إعادة تعيين إحداثيات Y بناءً على موضع اللاعب
        coins_collected = [False] * 5  # إعادة تعيين حالة جمع العملات

    # رسم العملات التي لم يتم جمعها
    for i, coin_rect in enumerate(coin_rects):
        if not coins_collected[i]:  # رسم العملة إذا لم تُجمع
            coin_images = coin_image[moves_coins]  # اختر الإطار الحالي من الرسوم المتحركة
            screen.blit(coin_images, coin_rect)

    # تحديث الرسوم المتحركة للعملات
    moves_coins += 1
    if moves_coins >= len(coin_image):
        moves_coins = 0

    # التحقق من جمع جميع العملات
    if coins_collected[-1]:
        coins_x = screenw + 200  # إعادة تعيين إحداثيات X للعملات
        coins_collected = [False] * 5  # إعادة تعيين حالة جمع العملات

        # تحديد موضع Y الجديد للعملات بناءً على موضع اللاعب
        if y >= 500:
            coins_y = pos_rand_coins_y
        elif y <= 300:
            coins_y = neg_rand_coins_y
        else:
            coins_y = y + 200

box_collide = [False,False,False]
def draw_boxes():
    global x2, box_collide

    x2 -= 15
    rects36 = [
        pygame.Rect(x2 + 5, 100, 200, 190),
        pygame.Rect(x2 + 5, 300, 200, 190),
        pygame.Rect(x2 + 5, 500, 200, 190)
    ]

    # تحقق من التصادم وتحديث حالة الصناديق
    for i, box_rect in enumerate(rects36):
        if player_rect.colliderect(box_rect) and not box_collide[i]:
            box_collide[i] = True  # جمع الصندوق
        if box_rect.x ==-100:
            box_collide[i] = True
            data['coin_num'] += 5
    # استخدم حالة الصناديق قبل رسمها
    for i, box_rect in enumerate(rects36):
        if not box_collide[i]:  # لا ترسم الصندوق إذا تم جمعه
            screen.blit(boxs, box_rect)
...
jump = [pygame.image.load(f'jump/Comp 2_{g :04}.png') for g in range(1,36)]
jump2 = [pygame.transform.scale(pygame.image.load(jump1), (100, 140)) for jump1 in image_paths]
...
def game_screen():
    global jump_speed_animation,rect,coins_x,coins_y,box_collide,coins_collected,if_dead,dead,num_dead,num_life,life,rects33 ,player_think,moves_player_think,background_scroll_Big,Space,pouse,bg_scroll,scroll_speed,background,x, y, x1, y1,x2, ask, enemy_moves, moves, rockk_x, rockk_y, shild_moves, draw_circle, player_rect, can_damjed,can_ask,random_question,rock,liqued_brain,y_question
    if not options_pouse:
        ...
        if y_question:
            ...
            type2()
        if is_jump:
            if jump_speed_animation < len(jump2):
                screen.blit(jump2[jump_speed_animation], (100, 300))
                jump_speed_animation += 1
            else:
                is_jumping = False
        if rock:
        ...
        font = pygame.font.Font(r'fonts/comic.ttf', 64)
        text = font.render(str(data['coin_num']), True, (255, 255, 255))
